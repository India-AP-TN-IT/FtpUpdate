...
namespace FTP_Downloader
{
    public class FtpHelper
    {
        ...
        /// Ftp 파일 확인하여 다운로드
        /// </summary>
        public bool FileDownLoad(ftpinfo ftpFile, bool passiveMode, ref string strErr)
        {
        ...
        private void Download(ftpinfo ftpFile, bool passiveMode)
        {
        ...
        /// <returns></returns>
        private List<ftpinfo> GetFullFilesListSub(int depth, List<ftpinfo> files, string subFolder, bool passiveMode)
        {
            string path = "";
            path = subFolder + "/";
            ...
            foreach (string line in lines)
            {
                ...
                {
                    files = GetFullFilesListSub(item.depth, files, path + item.filename, passiveMode);
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public List<ftpinfo> GetFullFilesList(bool passiveMode = false)
        {
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (item.fileType == directoryEntryTypes.directory)
                    {
                        subFiles = GetFullFilesListSub(item.depth, files, path + item.filename, passiveMode);
                        if (subFiles != null)
                        {
                            files = subFiles;
                        }
